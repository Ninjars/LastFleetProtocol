...
5. **Two lifecycle hooks on `Ship`, with distinct purposes.**
   - `onLifecycleTransition: ((Ship, ShipLifecycle) -> Unit)?` fires at *every* state change (`Active → LiftFailed`, `Active → Destroyed(HULL)`, `LiftFailed → Destroyed(LIFT_FAILURE)`). `GameStateManager` subscribes to this hook to re-tally match results via `playerShips.none { it.lifecycle is ShipLifecycle.Active }` / `enemyShips.none { ... }`. This is what makes R19's "counted as destroyed at Keel failure" hold — the match-result fires at the `LiftFailed` transition even though the ship's actor remains in the scene during drift.
   - `onDestroyedCallback: ((Ship, DestructionCause) -> Unit)?` fires *only* at the terminal transition into `Destroyed`. Its purpose is actor cleanup (remove from backing list, `actorManager.remove`) and the cause-tagged `println` destruction log.
   Rationale for two hooks: a single callback with overloaded meaning would conflate "match-result moment" with "actor-removal moment" — they're the same event for `Destroyed(HULL)` but *different* events for the lift-failure flow (match ends at `LiftFailed` entry, actor removed ~3s later). Two hooks keep each concern explicit and let `GameStateManager` keep `LiftFailed` ships in the backing list during drift without corrupting the tally.

6. **`DestructionCause` is an enum on the destruction callback, not a new combat-log subsystem.**
   `HULL` and `LIFT_FAILURE` cover R19 legibility. `GameStateManager` logs `"[combat] $teamId ship destroyed: $cause"` via `println`. A visible combat log UI is deferred; the plumbing needed to support one is already in place once the cause flows through.
...
- `[T]` = `onLifecycleTransition` hook fires.
- `[M]` = match-result re-tally side-effect (via `onLifecycleTransition`). Fires at `LiftFailed` entry AND at `Destroyed(HULL)` entry — both are R19 "moment of destruction" events.
- `[D]` = `onDestroyedCallback` hook fires. Actor cleanup + cause-tagged `println` destruction log.
...
- [ ] **Unit 3: Keel arc routing and LIFT_FAILED lifecycle**
  ...
  **Files:**
  ...
  - Modify: `features/game/impl/src/commonMain/kotlin/jez/lastfleetprotocol/prototype/components/game/managers/GameStateManager.kt` — wire *both* hooks on ship creation. `onLifecycleTransition` re-tallies match results via `playerShips.none { it.lifecycle is ShipLifecycle.Active } / enemyShips.none { ... }` — so victory fires at `LiftFailed` entry, not at drift-end. `onDestroyedCallback` handles actor cleanup (`actorManager.remove`, remove from backing team list) plus the cause-tagged `println`. `LiftFailed` ships remain in the backing team list until the terminal transition — the match-result decision uses the `none { is Active }` filter over the backing list instead.
  ...
  **Approach:**
  ...
  - **Every transition fires `onLifecycleTransition`; only transitions *into* `Destroyed` fire `onDestroyedCallback`.** The two-hook split is the Key Technical Decision 5 rationale — match-result re-tally is a lifecycle concern, actor-cleanup is a destruction concern. `GameStateManager` wires both. R19's "counted as destroyed at Keel failure moment" is satisfied by the `onLifecycleTransition`-driven re-tally at `LiftFailed` entry; actor cleanup waits for the terminal `Destroyed` transition so the ship is visibly present during drift.
...
- **Interaction graph:** `Ship` gains a second lifecycle hook (`onLifecycleTransition`) alongside the renamed `onDestroyedCallback: (Ship, DestructionCause) -> Unit`. `GameStateManager` wires both: transition-driven re-tally of match results (fires at `LiftFailed` entry and at terminal `Destroyed` entry) and destruction-driven actor cleanup (fires only at terminal `Destroyed`). The backing `playerShips`/`enemyShips` lists retain `LiftFailed` ships during drift — the match-result decision uses `none { it.lifecycle is Active }` over the backing list, not list-emptiness. `isValidTarget()` moves from `!isDestroyed` to `lifecycle is Active`, which affects turret target acquisition and navigator combat-target validity. Turrets are gated at the ship level (`Ship.update` skips turret update calls when `lifecycle !is Active`) rather than self-checking, sidestepping the Kubriko ActorManager child-update-ordering race. The damage path gains a polygon-intersection pre-check before the arc router's existing logic, with KEEL-first overflow cascading in a deterministic order.
...
- **State lifecycle risks:** The `LiftFailed` drift window is the first non-binary ship state in the game. The key risk is double-counting: a `LiftFailed` ship is both visible on the battlefield and "already destroyed for tally purposes". Handled by the two-hook split — `onLifecycleTransition` drives the match-result re-tally at every transition (including `Active → LiftFailed`), while `onDestroyedCallback` drives actor cleanup only at terminal transitions. The backing team list retains `LiftFailed` ships during drift; the tally filters them out via `none { it.lifecycle is Active }`. Reactor and Keel paths both eventually converge on `Destroyed`, but only the reactor path triggers actor cleanup immediately.
- **API surface parity:** `Ship.isDestroyed` is deleted. Every caller migrates to `lifecycle is ShipLifecycle.Destroyed` (or the inverted `is Active` for the common case). Callsites: `InputController`, `DebugVisualiser`, `Ship.isValidTarget`, `GameStateManager` (which also gains the new `onLifecycleTransition` wiring). Turrets are *not* in this list — they're gated at the ship level (parent skips their update calls when `lifecycle !is Active`) rather than self-checking parent state.
...
| Match-result logic miscounts during drift — a `LiftFailed` ship is "on the battlefield" but the opposing team should already have won | Two-hook architecture: `onLifecycleTransition` drives match-result re-tally via `playerShips.none { it.lifecycle is Active } / enemyShips.none { ... }` — fires at `LiftFailed` entry and at terminal `Destroyed` entry. Actor cleanup (via `onDestroyedCallback`) runs separately at terminal `Destroyed` only. Test asserts victory fires at the Keel-destruction moment, not at despawn, and does not re-fire on the subsequent `Destroyed(LIFT_FAILURE)` transition. |
